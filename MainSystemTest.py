...
class USVController:
    ...
    def set_servo(self, servo_pin, target_pwm, see_motor_output=0, ramp_time=0.5, update_interval=0.05):
        """
        Servo çıkışındaki PWM değeri ani değişiklikler yerine lineer ramp kullanarak,
        ayrı bir iş parçacığında (thread) günceller.

        Parametreler:
          servo_pin: Hedef servo pin numarası
          target_pwm: Ulaşılmak istenen PWM değeri
          see_motor_output: Eğer 1 ise, motor çıkışlarını ekrana yazdırır.
          ramp_time: Geçişin tamamlanması için ayrılacak süre (saniye cinsinden)
          update_interval: Her adım arasında bekleme süresi (saniye cinsinden)
        """
        if not hasattr(self, 'last_pwm'):
            self.last_pwm = {}
        # Önceki PWM değeri yoksa, direk hedef değeri kullan
        current_pwm = self.last_pwm.get(servo_pin, target_pwm)

        # Yeni bir iş parçacığı başlatıyoruz, böylece ramping paralel çalışır.
        threading.Thread(
            target=self._ramp_servo,
            args=(servo_pin, current_pwm, target_pwm, ramp_time, update_interval, see_motor_output),
            daemon=True  # Ana program kapanırken thread'ler de kapanır.
        ).start()

    def _ramp_servo(self, servo_pin, current_pwm, target_pwm, ramp_time, update_interval, see_motor_output):
        """
        Belirtilen servo için mevcut PWM değerinden hedef PWM değerine,
        belirtilen süre ve adım aralıkları ile lineer ramping uygular.
        """
        # Fark çok küçükse doğrudan hedef değeri gönderelim.
        if abs(target_pwm - current_pwm) < 5:
            ...
            self.last_pwm[servo_pin] = target_pwm
            if see_motor_output:
                self.print_motor_outputs()
            return

        steps = max(1, int(ramp_time / update_interval))
        step_size = (target_pwm - current_pwm) / steps

        for i in range(1, steps + 1):
            intermediate_pwm = int(current_pwm + step_size * i)
            self.master.mav.command_long_send(
                self.master.target_system,
                self.master.target_component,
                mavutil.mavlink.MAV_CMD_DO_SET_SERVO,
